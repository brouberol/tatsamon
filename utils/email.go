...
const templAlert = `Alert from Tatmon,
...
//SendAlertEmail send an alert mail
func SendAlertEmail(title, text string) error {
	return sendEmail(templAlert, "Tatmon - Alert - "+title, text)
}

func sendEmail(templ, subject, text string) error {

	toUser := viper.GetString("smtp_to")
	return sendEmailGeneric(templ, subject, toUser, text)
}

func sendEmailGeneric(templ, subject, toUser, text string) error {
	...
	paramEmail := &paramEmail{TextMail: text}
	...
	if viper.GetBool("smtp_tls") {
		...
		if err != nil {
			log.Errorf("Error with smtp.dial: %s", err.Error())
		}
		defer c.Close()
	}
