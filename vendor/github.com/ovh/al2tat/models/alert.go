...
func (alarm *Alert) PostToTatEngine(tatUsername, tatPassword, topic string) (models.Message, error) {
	...
	m := messageJSON{Text: text, Action: alarm.Action, IDReference: alarm.IDReference}
	...
	if alarm.Alert == "AL" && alarm.IDReference == "" {
		log.Debugf("New AL root msg, add label open on %s", created.Message.ID)
		alarm.addLabel(topic, tatUsername, tatPassword, created.Message.ID, "open")
	} else if alarm.Alert == "UP" && alarm.IDReference == "" {
		alarm.addLabel(topic, tatUsername, tatPassword, created.Message.ID, "done")
		log.Debugf("New UP root msg, add label done on %s", created.Message.ID)
	}

	if len(alarm.Labels) > 0 {
		for _, label := range alarm.Labels {
			writeLabel(topic, tatUsername, tatPassword, created.Message.ID, label)
		}
	}
...
func AlertPurge(skip, limit int, tatUsername, tatPassword, topic string) error {
	...
	msgs, err := utils.GetWantReturn(fmt.Sprintf("/messages%s?skip=%d&limit=%d&onlyMsgRoot=true", topic, skip, limit), tatUsername, tatPassword)
	if err != nil {
		log.Errorf("Error with getting list root err:%s", err.Error())
		return err
	}
	err = json.Unmarshal(msgs, &messagesRoot)
	if err != nil {
	...
	for _, m := range messagesRoot.Messages {
		...
		msgs, err := utils.GetWantReturn(fmt.Sprintf("/messages%s?skip=%d&limit=%d&treeView=onetree&idMessage=%s", topic, 0, 1, m.ID), tatUsername, tatPassword)
		if err != nil {
...
func (alarm *Alert) addLabel(topic, tatUsername, tatPassword, idMessage, label string) {
	...
	l := Label{Text: label, Color: color}
