...
func writeLabel(topic, tatUsername, tatPassword, idMessage string, label Label) {
...
func messagesList(tatUsername, tatPassword, criteriaTopic, criteriaText, criteriaTag, criteriaLabel string, criteriaDateMinUpdate, criteriaDateMaxUpdate string) (messagesJSON, error) {
	...
	msgs, err := utils.GetWantReturn(fmt.Sprintf("/messages%s?skip=%d&limit=%d%s&treeView=onetree", criteriaTopic, 0, 1, c), tatUsername, tatPassword)
	if err != nil {
		return messages, err
	}
